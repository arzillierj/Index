...
- **HK observers must be stoppable.** Every new `HKObserverQuery` / `HKAnchoredObjectQuery` needs to be retained on the service so a future Settings toggle-off can call `store.stop(query:)`. Per DQ10: stopping HK delivery does NOT delete previously-imported HK rows.
- **Calorie target is linear: `caloriesBase = TDEE + calorieAdjustmentKcal`.** No silent clamp, no near-BMR floor, no scaling. The user's slider input IS the contract — if they pick -1000, the math returns -1000. Safety lives at the input boundary (the `CalorieAdjustmentEditSheet` slider is bounded to `[-1000, +1000]` in 50-kcal steps), not in the engine. Reason: a prior `max(1200, BMR * 1.1)` floor silently overrode the user's deficit — for a typical 2674 TDEE + -1000 adjustment it produced 2139 instead of 1674. If a future safety floor is added, it must (a) sit well below BMR (a true floor, not a soft clamp near BMR), AND (b) surface in the UI when it engages so the user can see the number doesn't match their setting.
